...
if directed==True:
    def save_checkpoint(state, filename=f"processed_data/GCN_diffpool-{sequence_length}_directed.pth.tar"):
        print("__Saving Checkpoint__")
        torch.save(state, filename)

else:
    def save_checkpoint(state, filename=f"processed_data/GCN_diffpool-{sequence_length}_undirected.pth.tar"):
        print("__Saving Checkpoint__")
        torch.save(state, filename)
...
if load_model == True and directed == True:
    load_checkpoint(torch.load(f"processed_data/GCN_diffpool-{sequence_length}_directed.pth.tar", map_location=device))

elif load_model == True and directed == False:
    load_checkpoint(torch.load(f"processed_data/GCN_diffpool-{sequence_length}_undirected.pth.tar", map_location=device))
